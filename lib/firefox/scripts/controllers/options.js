...
import bootstrapcolorpicker from '../../../core/assets/js/bootstrap-colorpicker.min.js';
...
export class OptionCtrl {
  ...
  getData() {
    browser.storage.local.get(null).then((data, err) => {
      console.log(data);
  ...
  save(data, successMessage) {
    browser.storage.local.set(data, (error) => {
      if (error) {
  ...
  deleteActivationData() {
    ...
    // using browser storage because we don't want any message to be displayed
  ...
  setTextColor(data) {
    ...
    // calling browser.storage directly because we don't want success message
    browser.storage.local.set({ translatedWordStyle: this.cssOptions.join(';') });
  }
  ...
  backupKeys() {
    browser.storage.local.get(['googleTranslatorApiKey', 'azureTranslatorApiKey', 'yandexTranslatorApiKey']).then((data, err) => {
      saveFile(data, 'mtw_keys.txt');
    });
  }

  backupAll() {
    var saveConfig_ = browser.storage.local.get(null);
    saveConfig_.then((data, err) => {
