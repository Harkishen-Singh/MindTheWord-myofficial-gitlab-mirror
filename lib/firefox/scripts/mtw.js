...
export class ContentScript {
  ...
  initialize(res) {
    ...
    this.custom_icon.push(browser.runtime.getURL('assets/img/speak.png'));
    this.custom_icon.push(browser.runtime.getURL('assets/img/learnt.png'));
    this.custom_icon.push(browser.runtime.getURL('assets/img/visual.png'));
    this.custom_icon.push(browser.runtime.getURL('assets/img/info.png'));
    this.custom_icon.push(browser.runtime.getURL('assets/img/save.png'));
    this.custom_icon.push(browser.runtime.getURL('assets/img/blacklist.png'));
    // images for hover cards
    this.custom_icon.push(browser.runtime.getURL('assets/img/rare.png'));
    this.custom_icon.push(browser.runtime.getURL('assets/img/occasionally.png'));
    this.custom_icon.push(browser.runtime.getURL('assets/img/common.png'));
    this.custom_icon.push(browser.runtime.getURL('assets/img/tick_green.png'));
    this.custom_icon.push(browser.runtime.getURL('assets/img/cross_red.png'));
        ...
        this.getCommonWords().then((cwList) => {
          if(cwList){
            ...
            translator.getTranslations(cwList)
              .then((tMap) => {
                this.cwMap = tMap;
                browser.storage.local.set({ cwAvailable: true, cwMap: this.cwMap });
  ...
  injectCSS(cssStyle) {
    try {
      ...
      style.type = 'text/css';
      style.href = browser.runtime.getURL('/assets/css/MTWStyles.css');
  ...
  getCommonWords(){
    var commonWordsURI = browser.runtime.getURL('./common/' + this.srcLang + '.json');
  ...
  processTranslations(translationMap, userDefinedTMap, cwMatch) {
    ...
    //for quiz feature
    browser.storage.local.set({ 'translatedWordsForQuiz': JSON.stringify(this.filteredTMap) });
    ...
    var wordsEveryDay = [];
    browser.storage.local.get('nOfWordsTransEveryDay').then((result, err) => {
    ...
    if (!this.userDefinedOnly) {
      ...
      browser.storage.local.set({ 'stats': this.stats });
      browser.storage.local.set({ 'numberOfTranslatedWords': JSON.stringify(this.numberOfTranslatedWordsOnPage) });
    }

    //number of words translated for active pattern
    browser.storage.local.get('savedPatterns').then((result, err) => {
    ...
    var wordFrequencyPromise = new Promise((resolve, reject) => {
      ...
      browser.storage.local.get(['wordFrequencyCorpus','totalWordsperLanguage']).then((wordObjectList, err) => {
    ...
    wordFrequencyPromise.then((nativeWords) => {
      ...
      browser.storage.local.get(['learntWords', 'translatedWordsForQuiz']).then((wordsListObj, err) => {
        ...
        for (let i = 0; i < buttonList.length; i++) {
          if(!buttonList[i].classList.contains('done')){
            buttonList[i].addEventListener('click', function() {
              ...
              browser.storage.local.get(['learntWords']).then((result, err) => {
        ...
        for (let i = 0; i < buttonList.length; i++) {
          if(!buttonList[i].classList.contains('done')){
            buttonList[i].addEventListener('click', function() {
              ...
              browser.storage.local.get('savedTranslations').then((result, err) => {
        ...
        for (let i = 0; i < buttonList.length; i++) {
          if(!buttonList[i].classList.contains('done')){
            buttonList[i].addEventListener('click', function() {
              ...
              browser.storage.local.get('userBlacklistedWords').then((result, err) => {
                ...
                undo.addEventListener('click', function() {//Undo the change
                  var toUndo = undo.getAttribute('snack_original');
                  browser.storage.local.get('userBlacklistedWords').then(re => {
...
function connection(status, url = '', time = '') {
  ...
  if (status === 'success') {
    ...
    mtwConnectionHead.innerHTML = '<div><strong>MTW:</strong>  <span id="mtw-reconnect-time">Could not connect to Translator Service Reconnecting in '
    + tempTime + 's  &nbsp;....&nbsp;</span><span id="mtw-reconnect-now"  style="cursor:pointer;"><strong>Reconnect Now</strong></span> </div> <span id="mtw-connection-cross" style="position: absolute; right: 1em; font-size: 1.2em;cursor:pointer;">✕</span> </div>';
    if (status === 'fail')
