...
export class ContentScript {
  ...
  fetchTranslations(countedWords) {
    ...
    var translator = this.getTranslator();
    testConnection(translator.testurl);
    this.getCachedTranslations(cwNotMatch).then(cachedMaps => {
      let sizeCWMap = Object.keys(cwNotMatch).length,
        sizeCacheMap = Object.keys(cachedMaps).length;
      console.warn('cached percent: ' + (sizeCacheMap/sizeCWMap)*100 + '%');
      let forTranslator = {};
      for (var word in cwNotMatch) {
        if (!(word in cachedMaps)) {
          forTranslator[word] = cwNotMatch[word];
        }
      }
      if (forTranslator) { // prevent use of translator in 100% caching
        translator.getTranslations(forTranslator)
          .then((tMap) => {
            this.cacheTranslations(tMap); // cache only newer translations
            Object.assign(tMap, cachedMaps); // merge both translated objects into tMaps
  ...
  /**
   * Saves caches for translation map for efficient translations.
   * @param {Object} - map for words along with their translations.
   */
  cacheTranslations(tMap) {
    chrome.storage.local.get('cacheTranslations', result => {
      let cached = JSON.parse(result.cacheTranslations),
        map = this.srcLang + '-' + this.targetLanguage,
        mapInv = this.targetLanguage + '-' + this.srcLang;
      for (var trans in tMap) {
        try {
          if (trans !== tMap[trans]) { // prevent caching of untranslated maps
            cached[map][trans[0]][trans] = tMap[trans];
            cached[mapInv][tMap[trans][0]][tMap[trans]] = trans;
          }
        } catch(e) {
          try {
            cached[map][trans[0]]= {};
            cached[mapInv][tMap[trans][0]] = {};
            cached[map][trans[0]][trans] = tMap[trans];
            cached[mapInv][tMap[trans][0]][tMap[trans]] = trans;
          } catch(e) {
            cached[map] = {};
      ...
      chrome.storage.local.set({ 'cacheTranslations': JSON.stringify(cached) });
    });
  }

  /**
   * Retrives cache for translation map.
   * @param {Object} - map for words count.
   * @returns {Promise} - promise resolving translation cache
   */
  getCachedTranslations(map) {
    return new Promise((resolve, reject) => {
      chrome.storage.local.get('cacheTranslations', result => {
        let cachedData = JSON.parse(result.cacheTranslations),
          path = this.srcLang + '-' + this.targetLanguage,
          cacheTMaps = {};
        if (cachedData)
          for (var word in map) {
            try {
              if (cachedData[path][word[0]] !== undefined) { // skip exceptions of not existing initials in the cache
                let cachedMap = cachedData[path][word[0]];
                for (var wd in cachedMap) {
                  if (wd === word) {
                    cacheTMaps[wd] = cachedMap[wd];
                    break;
                  }
                }
              }
