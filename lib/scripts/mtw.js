      ...
      for (let a = 0; a < paragraphs.length; a++) {
        ...
        for (let i = 0; i < translatedWords.length; i++) {
          ...
          if (!(wordsArray.includes(word))) { // if word is not learnt
            ...
            hoverHTML += '<img class="moreInformation" data-original="' + translatedWords[i].getAttribute('data-original') + '" src="'+ this.custom_icon[11] +'">';
              ...
              if (srcLang === 'en') {
                ...
                result.then(res => {
                  ...
                  if (dicEntries.length) {
                    ...
                    let details = '<hr/><span class="hc-info-details"><b>Details Not Available</b></span>';
                    parentElement.innerHTML += details;
                  }
                });
              } else {
                ...
                translatorInv.getTranslations(wordArr).then(engArr => {
                  ...
                  result.then(res => {
                    let dicEntries = JSON.stringify(res),
                        translationArr = [
                          'Type', 'Uses', 'Word', 'Synonyms'
                        ], exceptions = [
                          'tr', 'ts', 'syn', '[', ']', 'def', 'head', 'text', 'pos'
                        ], map = {
                          'Type': 1, 'Uses': 1, 'Word': 1, 'Synonyms': 1
                    };
                    ...
                    translator.getTranslations(map).then(trans => {
                      ...
                      if (dicEntries.length) {
                        ...
                        let details = '<hr/><span class="hc-details"><b>Details Not Available</b></span>';
                        parentElement.innerHTML += details;
                      }
                    });
