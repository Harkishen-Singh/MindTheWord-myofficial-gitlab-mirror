...
export class ContentScript {
  ...
  filterDoms() {
    var dom = this.allparagraphs,
        filteredDoms=[],
        safePermit = 3000, // higher than this causes performance issues
        domLength = dom.length;
    var loopStart = domLength > safePermit ? domLength - safePermit : 0;
    if (loopStart !== 0) {
      console.warn('DOMs beyound safe permits. Skipping older DOMs..');
    }
    for (let i=loopStart; i< domLength; i++) {  // execute only the newer 3000 DOMs only.
  ...
  initialize(res) {
    ...
    this.filterDoms();
    console.warn(this.allparagraphsFiltered)
    ...
    if (blacklistWebsiteReg.test(document.URL) && res.blacklist !== '()') {
      ...
      window.onload = () => {
        ...
        if (MTWTranslator.finishTranslation) {
          ...
          console.log('previousParagraphs length : '+previousParagraphs.length)
            ...
            List.forEach(node => {
              try {
                let e = node.addedNodes[0].classList, // each event emits only a single node
                  ...
                  if (node.type === 'childList' && len && allow && MTWTranslator.finishTranslation &&
                      !(e.contains('hc-preview') || e.contains('hc-details') || e.contains('mtwTranslatedWord'))) {
                    scanCount++;
