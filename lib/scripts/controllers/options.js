...
export class OptionCtrl {
  ...
  changeApiKey(translator) {
    ...
    switch (translator) {
      ...
      case 'Google':
        ...
        break;
      case 'Bing':
        if (/^\s*$/.test(this.bingTranslatorApiKey.clientSecret) && this.translatorService === 'Bing') {
          this.deactivatePatterns();
          this.toggleDoNotTranslate();
        }
        this.save({
  ...
  hasKey(translator) {
    switch (translator) {
      ...
      case 'Bing':
        return /^\s*$/.test(this.bingTranslatorApiKey.clientSecret);
      default:
  ...
  noKeys() {
    if (!/^\s*$/.test(this.googleTranslatorApiKey) || !/^\s*$/.test(this.yandexTranslatorApiKey) || !/^\s*$/.test(this.bingTranslatorApiKey.clientSecret)) {
      return false;
    }
    return true;
  }
